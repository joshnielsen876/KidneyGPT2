...
DATABASE_PATH = r'D:\buffalo data\project\chatgpt\KidneyGPT2\chat_data.db'
...
def insert_into_chat_history(user_id, session_id, user_msg, assistant_msg, master_schema, incremental_update, changelog):
    try:
        ...
        conn.commit()
        print("Data inserted into chat_history table successfully.")

        
        # Immediately query database to check if the insert was successful
        cursor.execute("SELECT * FROM chat_history WHERE user_id = ?", (user_id,))
        rows = cursor.fetchall()
        print("Data retrieved from chat_history table:")
        for row in rows:
            print(row)
